...
if [[ "$descarga" == "Video" ]]; then
	cd /home/$USER/Vídeos
	if yt-dlp --format 'bestvideo+bestaudio/best' --merge-output-format 'mp4' "$url" | zenity --progress; then
...
if [[ "$descarga" == "Audio" ]]; then
	cd /home/$USER/Música
	if yt-dlp --extract-audio --audio-format mp3 --audio-quality 192k -o '%(title)s.%(ext)s' "$url" | zenity --progress; then
